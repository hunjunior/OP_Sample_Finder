...
const MODEL_DIR = './pretrained_models/drum_classes/fft-model.model'
...
async function getLoopSamples() {
    if (filePath) {
        if (drumPrediction.length) {
...
function audioToArray(filepath, callback) {
    let msg = "";
    ...
    if (fileSizeInBytes < (MAX_FILE_SIZE * 1000000)) {
        fs.readFile(filepath, async (err, buf) => {
            if (err) {
                ...
                audioContext.decodeAudioData(resp, stereobuffer => {
                    
                    let buffer = stereoToMono(stereobuffer);

                    result = buffer.getChannelData(0);
                    let audioData16bit = new Int16Array(result);

                    return callback(audioData16bit, null);
                })
    
            }
        })
    } else {
        return callback("", 'Max file size is ' + MAX_FILE_SIZE + ' MB.');
    }
}
...
function getDrumSamples() {
    if (filePath) {
        pauseSample();
        startLoader("<b>Separating the drum track (this may take a few minutes)...</b><br>ドラムのループ素材の分離...");
        document.querySelector("#loading-msg").innerHTML = "Processing...";
        document.querySelectorAll("button").forEach((btn) => btn.disabled = true);
        client.invoke("getDrumTrack", filePath, async (error, drumTrackFilePath) => {
            if (error) alert(error);
                ...
                let buffer = await getAudioBuffer(trackFilePaths.drums);
                currentBufferData = buffer.getChannelData(0);
...
function handleNewFile(newFilePath) {
    ...
    /* audioToArray(filePath, (audioArray) => {
...
function addSample() {
    if ((currentView == 'zoom' && currentZoomRegion) || (currentView == 'primary' && currentRegion)) {
        let tempRegion = null;

        if (currentView == 'zoom') tempRegion = currentZoomRegion;
        else tempRegion = currentRegion;

        let startIndex = Math.round(tempRegion.start * 44100);
        let endIndex = Math.round(tempRegion.end * 44100);
...
function saveSample() {
    let outputFinalDir = dialog.showOpenDialog({ properties: ['openDirectory'] })[0];
    if (finalRegions.length) {
        ...
        getOpzObject((obj) => {
            ...
            let outputTempDir = "./temp/OP-Z_join_" + isoDate + ".aiff";
            outputFinalDir += "/OP-Z_JSON_" + isoDate + ".aif";

            convertFile(finalFilePath, outputTempDir, (success, error) => {
                if (error) {
                    alert(error);
                } else {
                    joinJSONtoAIFF(outputTempDir, outputFinalDir, obj, () => {
                        showCreatedMsg(outputFinalDir);
                    })
                }
            })
...
function convertFile(inputPath, outputPath, callback) {
    ...
    fs.readFile(inputPath, (err, data) => {
        ...
        audioContext.decodeAudioData(data, buffer => {
            ...
            fs.writeFile(outputPath, Buffer.from(chunk), function (err) {
                if (err) throw err;
...
function joinJSONtoAIFF(inputDir, outputDir, obj, callback) {
    fs.readFile(inputDir, (err, buf) => {
        if (err) throw err;
...
function selectView(view) {
    if (view == "primary") {
        ...
        wavesurferElement.style.border = "solid 3px #f57047";
        ...
        if (currentZoomRegion && wavesurferZoom) {
            ...
            wavesurferZoomElement.style.border = "solid 3px #f57047";
        ...
        if (currentFinalRegion && wavesurferFinal) {
            ...
            wavesurferFinalElement.style.border = "solid 3px #f57047";
            wavesurferElement.style.border = "solid 1px #6f6f6f";
            wavesurferZoomElement.style.border = "solid 1px #6f6f6f";
        }
    }
...
async function concatenateBufferToFile(filePath, bufferDataNew, baseLength, callback) {
    ...
    if (totalSamplesLength > (12 * 44100)) alert("The final audio sample has to be shorter than 12 seconds.");
    else {
        let audioContext = new AudioContext;
